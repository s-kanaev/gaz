var Page = {
  ...
  getPostModuleCode: function(module) {
    switch(module) {
      ...
      case 'feed_podcasts': return 'h';
      case 'feed_podcasts_top': return 'h';
      case 'feed_podcasts_recent': return 'h';
      ...
      while (index--) {
        ...
        if (inViewport) {
          if (!isPlaying && !isLoading) {
            ...
            cur.videoAutoplayStat = {video: params.video, launched: Date.now(), preloaded: _isPreloaded(params.video)};
          }
          break;
...
var Wall = {
  ...
  getOwnerDraft: function(ownerId) {
    ...
    return [draft.txt, draft.medias, true, {from: intval(draft.from), signed: intval(draft.signed), shareShowImg: intval(draft.shareShowImg)}, draft.bkg];
  },
  ...
  updateTimes: function (cont) {
    ...
    var timeNow = intval(Date.now() / 1000), toClean = [];
    timeNow -= cur.tsDiff;
  ...
  init: function(opts) {
    ...
    extend(cur, {
      ...
      deletedCnts: {own: 0, all: 0, archive: 0},
  ...
  openPosterEditor: function(customData) {
    ...
    text = text.replace(/<img[^>]*>/g, function(str) {
      var img = ce('div', { innerHTML: str }).firstChild;
      ...
      opts.onLoadMore = function() {
        ajax.post('al_friends.php', {
          ...
          onDone: function (html, newFrom) {
            ...
            if (html) {
              items = domChildren(ce('div', { innerHTML: html }));
            }
  ...
  friendsRecommLogAddVisited: function(mid, ref, trackCode) {
    ...
    visited[mid] = { ref: ref, trackCode: trackCode };
  ...
  showReorderRepliesTooltip: function(el, postId) {
    ...
    if (!pageCur['postRepliesReorderTT' + postId]) {
      ...
      var content = se(rs(template_tooltip || '', { postId: postId }));
